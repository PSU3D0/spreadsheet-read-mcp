...
| `op` | _(canonical dispatcher)_ | ALL | `operations.execute_operation` | mvp | Machine JSON mode; `--bind` is required and injected only for resource actions, while SheetPort schema/validate/normalize run as portable content actions | `crates/agent-spreadsheet/src/cli/mod.rs::run_machine_operation` | `crates/agent-spreadsheet/tests/canonical_operations.rs` |
...
| `screenshot_sheet` | legacy MCP screenshot tool remains unchanged | validates bounded names/ranges before rendering, confines output beneath the workbook workspace, and returns a content-addressed PNG handle without server paths | `screenshot_rendering` | low |
