...
| `op` | _(canonical dispatcher)_ | ALL | `operations.execute_operation` | mvp | Machine JSON mode; descriptor capability and input-schema branch metadata drive bind/fork/export policy, including portable SheetPort actions | `crates/agent-spreadsheet/src/cli/mod.rs::run_machine_operation` | `crates/agent-spreadsheet/tests/canonical_operations.rs` |
